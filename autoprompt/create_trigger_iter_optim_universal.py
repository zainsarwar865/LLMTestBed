...
def run_model(args):
    ...
    for i in range(args.iters):
        ...
        for model_inputs, labels in tqdm(train_loader):
            ...
            for token_to_flip in range(templatizer.num_trigger_tokens):
                ...
                candidates = hotflip_attack(averaged_grad[token_to_flip],
                                            ...
                                            filter=filter)
                # Need to normalize averaged_grad by batch_size, currently summing it up
                ...
                denom = 0
                # Baseline performance with current triggers
                # This is to check if during optimization, you have succeeded then you can stop and move to the next input//batch
                with torch.no_grad():   
                    predict_logits, model_inps = predictor(model_inputs, trigger_ids)
                ...
                for k, candidate in enumerate(candidates):
                    ...
                    with torch.no_grad():
                        predict_logits, model_inps = predictor(model_inputs, temp_trigger)
                        eval_metric = evaluation_fn(predict_logits, labels)
                        eval_acc_metric = compute_accuracy(predict_logits, labels)

                    candidate_scores[k] = eval_metric.sum()
                    candidate_accs[k] = eval_acc_metric
                
                # If loss is worse than current_loss than update the trigger tokens globally
        ...
        logger.info(f'Evaluation for iteration : {i}')
        numerator = 0
        numerator_acc = 0
        denominator = 0
        model.zero_grad()
        for model_inputs, labels in tqdm(dev_loader):
            ...
            numerator += evaluation_fn(predict_logits[0], labels).sum().item()
            numerator_acc += compute_accuracy(predict_logits[0], labels)
            denominator += labels.size(0)
        dev_metric = numerator / (denominator + 1e-13)
        dev_acc_metric = numerator_acc / (denominator + 1e-13)

        logger.info(f"Dev acc metric is : {dev_acc_metric}")
        logger.info(f'Trigger tokens: {tokenizer.convert_ids_to_tokens(trigger_ids.squeeze(0))}')
        logger.info(f'Dev metric: {dev_metric}')

        if dev_metric <= best_dev_metric:
            logger.info('Best attack performance so far')
            best_trigger_ids = trigger_ids.clone()
            best_dev_metric = dev_metric

        
        if dev_acc_metric <= best_dev_acc_metric:
            logger.info('Lowest accuracy so far')
