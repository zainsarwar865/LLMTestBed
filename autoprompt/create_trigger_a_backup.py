...
def hotflip_attack(averaged_grad,
    ...
    with torch.no_grad():
        ...
        #logger.info(f"min is :  {torch.min(gradient_dot_embedding_matrix)}")
        #logger.info(f"Argmin is : {torch.argmin(gradient_dot_embedding_matrix)}")
        #logger.info(f"Argmax is : {torch.argmax(gradient_dot_embedding_matrix)})")
        _, top_k_ids = gradient_dot_embedding_matrix.topk(num_candidates)
        if 3 in top_k_ids:
            logger.info("UNK is in top-k ids")
        #logger.info(filter[0:5])
        #logger.info(gradient_dot_embedding_matrix[0:5])
        #logger.info(f"Top-k ids are : {top_k_ids}")
        #logger.info(f"\n\n\n")
        #logger.info(gradient_dot_embedding_matrix)
    return top_k_ids
...
def run_model(args):
    ...
    if args.initial_trigger:
        ...
        initial_trigger = ["good", "morning", "to" , "you", "you"]
    ...
    logger.info(f"Filter values are : {filter} ")
    ...
    for i in range(args.iters):
        ...
        for model_inputs, labels in tqdm(train_loader):
            ...
            if averaged_grad is None:
                averaged_grad = grad.sum(dim=0) / bsz
            else:
                averaged_grad += grad.sum(dim=0) / bsz
            ...
            # Some other weird baseline shit
            with torch.no_grad():   
                predict_logits = predictor(model_inputs, trigger_ids)
            ...
            for i, candidate in enumerate(candidates):
                ...
                candidate_accs[i] += eval_acc_metric
            ...
            if (candidate_accs > current_acc).any():
                logger.info('Better trigger with higher acc detected.')
                best_candidate_acc = candidate_accs.max()
                best_candidate_idx = candidate_accs.argmax()
                #trigger_ids[:, token_to_flip] = candidates[best_candidate_idx]
                logger.info(f'Train acc metric: {best_candidate_acc / (denom + 1e-13): 0.4f}')
        ...
        for model_inputs, labels in tqdm(dev_loader):
            ...
            numerator_acc += compute_accuracy(predict_logits, labels)
            denominator += labels.size(0)
        dev_acc_metric = numerator_acc / (denominator + 1e-13)    
        dev_metric = numerator / (denominator + 1e-13)

        logger.info(f'Trigger tokens: {tokenizer.convert_ids_to_tokens(trigger_ids.squeeze(0))}')
        logger.info(f'Dev metric: {dev_metric}')
        logger.info(f'Dev acc metric: {dev_acc_metric}')
        ...
        if dev_acc_metric > best_dev_acc_metric:
            logger.info('Best accuracy so far')
            best_dev_acc_metric = dev_acc_metric
    ...
    logger.info(f'Best dev acc metric: {best_dev_acc_metric}')
    if args.print_lama:
...
if __name__ == '__main__':
    ...
    if args.debug:
        ...
        level = logging.INFO
    logging.basicConfig(filename="logs.txt",level=level)
