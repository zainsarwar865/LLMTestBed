...
def run_model(args):
    ...
    if args.initial_trigger:
        ...
        initial_trigger = ["good", "bad", "ugly", "the"]
    ...
    for i in range(args.iters):
        ...
        for model_inputs, labels in tqdm(train_loader):
            ...
            for token_to_flip in range(templatizer.num_trigger_tokens):
                ...
                if(eval_acc_metric == 1):
                    if(new_example):
                        total_samples+=1
                else:
                    logger.info(f"Already adv trigger ids are : {trigger_ids}")
                    break
                new_example=False
    ...
    #result = total_incorrect / total_samples + 1e-13
    #logger.info(f"Attack success rate is : {result}")
